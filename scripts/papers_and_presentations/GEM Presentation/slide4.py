...
for i,name in enumerate(subject_names):
    ...
    default_cycler = (cycler(color=['r', 'g', 'b', 'c','m','y']) + \
                      cycler(alpha=[0.0, 0.0, 0.0, 0.4, 0.4, 0.4]))

    plt.rc('lines', linewidth=4)
    plt.rc('axes', prop_cycle=default_cycler)

    y = file_numpy + zero_foot_angle_offset
    
    if i > 0:
        ax.plot(x_axis,y, label='_nolegend_')
    else:
        ax.plot(x_axis,y)
...
model_output_offset = model_output + zero_foot_angle_offset #Fix to center the 
average_output_offset = average_output + zero_foot_angle_offset

labels = ['person 1','Average Fit Model']

#Average fit plot
ax.plot(x_axis, average_output_offset, linewidth=12)


#Plot to include the model output 
#ax.plot(x_axis, model_output_offset, linewidth=12)
...
plt.savefig('only_average.png', transparent=True)
#plt.savefig('only_differences.png', transparent=True)
#plt.savefig('avg_plus_differences.png', transparent=True)
