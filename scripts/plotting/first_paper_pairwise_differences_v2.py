...
gait_state_ISA_median = {state:df[df['Test'] == baseline_test].groupby(
                         gait_state_noise_list).mean()[state].median()
                         for state
                         in gait_states}
...
for df_temp in [df,df_optimal]:
    ...
    for state,df_name,df_name_norm in zip(gait_states,gait_state_diff,
        ...
        #Calculate the pairwise difference per trial and subject
...
max_phase_rmse = np.max(df['Phase RMSE Diff'])
max_stride_length = np.max(df['Stride Length RMSE Diff'])
max_ramp = np.max(df['Ramp RMSE Diff'])
#Create a new column that is the cost function
df['cost_func'] = np.power(df['Phase RMSE Diff'] / max_phase_rmse,2) \
                + np.power(df['Stride Length RMSE Diff'] / max_stride_length,2) \
                + np.power(df['Ramp RMSE Diff'] / max_ramp, 2)
#Optimal diff
df_optimal_idx = df['cost_func'].idxmin()
df_optimal_diff = df.iloc[[214]]  #Opitmal index found manually
print(df_optimal_diff)

print(f"{type(df_optimal)=}")
print(f"{type(df_optimal_diff)=}")
...
###################################
# This subsection makes interactive tick plot to see the distribution
# of differences
#Reset index to make it into a column
...
# #Temporary plot to debug the weird optimal results
# base = alt.Chart(df).mark_tick(
#     height=3, width = 25    
# ).encode(
...
#Group the optimal points
optimal_point_diff = alt.Chart(df_optimal_diff).mark_tick(
    color='orange', height=5, width=30
).encode(
    # x=alt.X('index:O',title=None,axis=None)
)
...
def create_subplot(diff_label, diff_label_norm,state_index):
    ...
    text_annotation_diff = alt.Chart(df_optimal_diff).mark_text(
        align='right',
        dx=dx, #Nudges for the box plot  
        dy=dy  #Only do this when applying the df_optimal as the data
    ).encode(
    ...
    text_annotation_norm_diff = alt.Chart(df_optimal_diff).mark_text(
        align='left',
        dx=-dx, #Nudges for the box plot  
        dy=dy  #Only do this when applying the df_optimal as the data
    ).encode(
    ...
    #calculate the point where the optimal plot is 
    optimal_plot_diff = optimal_point_diff.encode(
        y=alt.Y(f'{diff_label}:Q',scale=scale,axis=None),
        # tooltip=f'{diff_label}:Q'
    )
    ...
    #Add the plots
    subplot = alt.layer(
        box_plot + text_annotation + text_annotation_diff,
        box_plot_norm + text_annotation_norm + text_annotation_norm_diff,
        optimal_plot,
        optimal_plot_diff
    ).encode(
        x=alt.X('something:N',title=f'p={p_value:0.2}',
