...
#Get a random output
num_outputs = 5
output_name = r01_dataset[0][1].columns[0:num_outputs]
...
#Make sure that the output size has the same number of dimensions as the things
assert test1.shape[1] == num_outputs
...
assert np.all(test2 == test_zero_gf1)
...
assert np.all(test1 != test3)
assert np.all(test2 != test3)
...
assert np.all(test_multi_1[0,:] == test1)
assert np.all(test_multi_1[1,:] == test1)
assert np.all(test_multi_2[0,:] == test2)
assert np.all(test_multi_3[0,:] == test3)
